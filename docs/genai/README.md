...
| `OLLAMA_MODEL` | `llama3.2` | Modele LLM local (Ollama) |
...
### Choix du modele LLM (par la mesure, 2026-06-26)
Le LLM est interchangeable (`OLLAMA_MODEL`). Decision prise sur le **dataset de test**
(39 cas, voir `tests/DATASET_TEST.md`), apres correction du prompt d'intention :

| Modele | Dataset (32 cas) | Latence intention | Narration |
|---|---|---|---|
| **`llama3.2` (3B, defaut)** | **32/32 — 100 %** | **~0,5 s** | bonne, ~0,8 s |
| `mistral:7b` | 24/32 — 75 % | ~1,0 s | tres complete, ~9 s |
| `gemma:7b` (Gemma 1) | (78 % avant correction) | ~2,3 s | bonne, ~5 s |

`llama3.2` est **retenu** : le dataset le donne **100 %** (le meme prompt degrade Mistral —
sensibilite au prompt propre au modele), et il est le **plus rapide**. Mistral reste
disponible via `OLLAMA_MODEL=mistral:7b`.
